...
def train():
    ...
    net_g_test = SRGAN_g(t_image,[],noise_std,is_variance=False,is_train=False, reuse=True)
    ...
    for epoch in range(0, n_epoch+1):
        ...
        if (epoch != 0):
            ...
            print("[*] save images")
            tl.vis.save_images(out0, [ni, ni], save_dir_gan+'/train_0_%d_noise_%d_last%d.png' % (epoch,args.noise,args.layer))
            tl.vis.save_images(out, [ni, ni], save_dir_gan+'/train_%d_noise_%d_last%d.png' % (epoch,args.noise,args.layer))

        ## save model
        if (epoch != 0):
            tl.files.save_npz(net_g0.all_params, name=checkpoint_dir+'/g_noise_%d_last%d_srgan.npz' %(args.noise,args.layer), sess=sess)
            tl.files.save_npz(net_d.all_params, name=checkpoint_dir+'/d_noise_%d_last%d_srgan.npz' %(args.noise,args.layer), sess=sess)

def evaluate():
    ## create folders to save result images
    save_dir = 'samples_rot_flip/set_14_evaluate'
    ...
    for imid_i in range(1):
        ...
        for rot in range(1):
            i+=1
            rotate_time=args.rot_time
            if args.flip==1:
                lr = cv2.flip(valid_lr_imgs_o[imid], 1)
                hr = cv2.flip(valid_hr_imgs_o[imid], 1)
            else:
                lr = valid_lr_imgs_o[imid]
                hr = valid_hr_imgs_o[imid]
            valid_lr_img = np.rot90(lr,rotate_time)
            valid_hr_img = np.rot90(hr,rotate_time)
            ...
            if i>1:
                ...
                '''
                net_g2 = SRGAN_g(tf.image.rot90(t_image,1),[],std,is_variance=False,is_train=False, reuse=True)
                net_g3 = SRGAN_g(tf.image.rot90(t_image,2),[],std,is_variance=False,is_train=False, reuse=True)
                net_g4 = SRGAN_g(tf.image.rot90(t_image,3),[],std,is_variance=False,is_train=False, reuse=True)
            ...
            tl.vis.save_image(np.rot90(out[0],4-rotate_time), save_dir+'/valid_gen_id_%02d_r%d_noise_%d_r0.png' % (imid,rotate_time,args.noise))
            out = sess.run(net_g.outputs, {t_image: [valid_lr_img]})
            ...
            #no noise
            #pdb.set_trace()
            out0 = sess.run(net_g.outputs, {t_image: [valid_lr_img]})
            if args.flip==1:
                img_save=cv2.flip(np.rot90(out0[0],4-rotate_time),1)
            else:
                img_save=np.rot90(out0[0],4-rotate_time)
            tl.vis.save_image(img_save, save_dir+'/valid_gen_id_%02d_r%d_r0_f%d.png' % (imid,rotate_time,args.flip))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noise', type=float, default=1, help='noise')
    parser.add_argument('--layer', type=int, default=2, help='last 2')
    ...
    parser.add_argument('--rot_time', type=int, default=0, help='rot_time')
    parser.add_argument('--flip', type=int, default=0, help='flip')
