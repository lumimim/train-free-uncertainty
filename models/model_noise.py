...
def gaussian_attention_noise_layer(input_layer, attention, std):
    noise = tf.random_normal(shape = tf.shape(input_layer), mean = 0, stddev = std, dtype = tf.float32)
    return input_layer + tf.multiply(noise,attention)


class GaussianNoise_attention_Layer(Layer):
    ...
    def __init__(
        ...
        if is_train is False:
            ...
            with tf.variable_scope(name):
                noise = tf.random_normal(shape = input_layer.get_shape(), mean=mean, stddev=stddev, seed=seed)
                self.outputs = self.inputs + tf.multiply(noise,attention)
            
            self._add_layers(self.outputs)

def SRGAN_g(t_image,variance,std,is_variance=False, is_train=False, reuse=False):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        if is_variance==True:
            n = gaussian_attention_noise_layer(n.outputs,std,variance)
        else:
            n = gaussian_noise_layer(n.outputs,std)
            n = InputLayer(n, name='in2')
        ...
        return n
def SRGAN_g0(t_image,variance,std,is_variance=False, is_train=False, reuse=False):
    """ Generator in Photo-Realistic Single Image Super-Resolution Using a Generative Adversarial Network
        feature maps (n) and stride (s) feature maps (n) and stride (s)
        """
    w_init = tf.random_normal_initializer(stddev=0.02)
